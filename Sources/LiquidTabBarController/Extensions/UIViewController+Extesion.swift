...
public extension UIViewController {
     var jDTabBarController : LiquidTabBarController? { get {
        if let parent = parent as? UINavigationController {
            if let controller = parent.parent as? LiquidTabBarController {
                return controller
            }
        }
            
        else if let parent = parent as? LiquidTabBarController {
            return parent
        }
        return nil
        }
